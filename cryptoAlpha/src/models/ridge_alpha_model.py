...
class RidgeAlphaModel:
    ...
    def build_label(self, panel_df: pd.DataFrame, price_col: str = "close") -> pd.DataFrame:
    ...
    def _fit_predict_impl(
        ...
        for i in range(self.train_window, len(dates)):
            ...
            y_train = np.asarray(train_df[label_col].to_numpy(dtype=float))
            ...
            if daily_model_dir is not None:
                ...
                model_fp = daily_model_dir / f"ridge_alpha_{test_ts:%Y%m%d_%H%M}.pkl"
