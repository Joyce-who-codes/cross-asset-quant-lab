...
class CoingeckoLoader:
    """
    CoinGecko NORMAL loader

    Supported path pattern:
    /COINGECKO_v3/NORMAL/{dataset}/{symbol}/*.csv
    ...
    @property
    def root(self) -> Path:
        return self.registry.coingecko_root

    def list_symbols(self, dataset: str = "COIN_MARKET_CHART/hourly") -> list[str]:
        ds_root = self.root / dataset
        if not ds_root.exists():
            return []
        return sorted([p.name for p in ds_root.iterdir() if p.is_dir()])

    def get_symbol_dir(self, dataset: str, symbol: str) -> Path:
        return self.root / dataset / symbol

    def exists(self, dataset: str, symbol: str) -> bool:
        return self.get_symbol_dir(dataset, symbol).exists()
    ...
    def load_symbol(
        ...
        symbol_dir = self.get_symbol_dir(dataset=dataset, symbol=symbol)
    ...
    def load_many_symbols(
        ...
        for symbol in symbols:
            if skip_missing and not self.exists(dataset, symbol):
                continue
    ...
    def load_many_market_chart_hourly(
        self,
        symbols: list[str],
        start_time: str | None = None,
        end_time: str | None = None,
    ) -> pd.DataFrame:
        return self.load_many_symbols(
            dataset="COIN_MARKET_CHART/hourly",
            symbols=symbols,
            start_time=start_time,
            end_time=end_time,
